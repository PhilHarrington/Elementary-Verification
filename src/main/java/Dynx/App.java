...
public class App  extends ListenerAdapter
{
    public static void main( String[] args ) throws Exception{
        ...
        FileInputStream serviceAccount = new FileInputStream("C:\\Users\\kolxoznik\\Desktop\\DiscordAuthBot\\aio-elementary-verification-firebase-adminsdk-6jqmi-cc9192a9ee.json");

        FirebaseOptions options = new FirebaseOptions.Builder()
                .setCredentials(GoogleCredentials.fromStream(serviceAccount))
                                    //SET THIS TO YOUR REALTIME DATABASE URL
                .setDatabaseUrl("https://aio-elementary-verification.firebaseio.com/")
                .build();
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                if(dataSnapshot.child("keys").child(message).exists()){
                    if(dataSnapshot.child("keys").child(message).getValue().equals("FALSE")){
                        ...
                        objMsgCh.sendMessage("Key verified! Welcome!").queue();
                        ...
                        List<Role> roles = milkFam.getRolesByName("User", true);
                    ...
                    /* 2 */  //  milkFam.getController().removeSingleRoleFromMember(milkFam.getMember(objUser), unverifiedRoles.get(0)).complete();
                        ...
                        objMsgCh.sendMessage("Sorry, but your key is already used.").queue();
                        return;
                    }
                } else {
                    objMsgCh.sendMessage("Invalid key").queue();
                }
